...
int parse_parameters(int argn, char **argv,
    ...
    struct arg_int *target_size         = arg_int0(NULL, "target_size", NULL, "Optional early-stop target MIS size. Stop once a solution of at least this size is found.");
    ...
    if (target_size->count > 0) {
        mis_config.target_size = target_size->ival[0];
    }
