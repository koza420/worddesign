...
void reduction_evolution<reducer>::perform_evolutionary(MISConfig & mis_config,
    ...
    // Early stop if the target size has already been found during initialization.
    if (mis_config.target_size > 0 &&
        mis_log::instance()->best_size() >= mis_config.target_size) {
        delete pool;
        pool = NULL;
        return;
    }
    ...
    do {
        ...
        // Early stop once the target size has been reached.
        if (mis_config.target_size > 0 &&
            mis_log::instance()->best_size() >= mis_config.target_size) {
            break;
        }
