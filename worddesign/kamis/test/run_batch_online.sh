...
output_base="$SCRIPT_DIR/results_online/output"
...
run_command() {
    ...
    echo "Using config: ${config}, time_limit: ${time_limit}"

    # Run the redumis command and redirect output and error to log file
    "$SCRIPT_DIR/../deploy/online_mis" "$input_file" --adaptive_greedy --time_limit="$time_limit" --seed="$seed" --output="$output_file" > "$log_file" 2>&1
}

# Export the variables for use in parallel execution
export output_base input_file time_limit
...
seq 501 510 | parallel -j 10 run_command
